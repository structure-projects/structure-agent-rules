...
| 形态 | 模块结构 | 持久化模式 | 适用 |
|---|---|---|---|
| **DDD 微服务**（**新项目默认**） | 7+1 模块（common/domain/infra/repository-mybatis/application/interfaces/boot/dependencies） | RepositoryFacade + Delegate + Entity/PO 分离 | structure-user / structure-org / structure-tenant / structure-resource 等业务中心 |
...
**每个业务服务是一个 monorepo**，包含 7+1 后端模块：
