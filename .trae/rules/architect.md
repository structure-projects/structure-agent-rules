# Architect — structure-projects 架构/设计规则速查

> 完整规则见 `prompts/architect.md`。本文件为 Trae IDE 速查版。

## 硬约束

- Maven `groupId` = `cn.structured`；npm scope = `@structure-projects`
- 包名：`cn.structure.*`（无 d）仅 `structure-common` / `structure-infra`；其余 `cn.structured.*`（含 `structure-security`）
...
| DDD 7+1 | `structure-ddd-template`（**默认推荐**） |
...
- RESTful；统一响应 `ResResultVO<T>`；统一错误码 `{X}ExceptionEnum`。
